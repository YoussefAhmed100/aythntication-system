...
exports.register =async function(req,res){
    try{
        ...
        return res.jeson({message:"User registred succesflly", user:{name:user.name  ,email:user.email} })
...
exports.login =async function(req,res){
    try{
        ...
        if(!user||!user.comparePassword(req.body.password))//TODO
        {
        ...
        return res.jeson({message:"User login succesflly", user:{name:user.name  ,email:user.email ,token:token} })
